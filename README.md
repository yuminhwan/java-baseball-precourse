...
## 🔥 구현할 기능 목록
### 컴퓨터 
- 1~9까지 서로 다른 수 이루어진 3자리의 수 만드는 기능 
### 플레이어 
- 숫자 입력 받는 기능 
  - 숫자 유효성 검사 기능
    - 잘못된 값을 입력할 경우 IllegalArgumentException 발생한 후 애플리케이션 종료
      - 3자리보다 짧거나 긴 길이의 숫자가 입력되는 경우
      - 1 ~ 9사이의 숫자가 아닌 경우 ( 0이나 문자인 경우 )
      - 서로 다르지 않는 수 입력되는 경우
### 컴퓨터와 플레이어 숫자 비교 기능
- 같은 수가 같은 자리에 있으면 스트라이크
- 같은 수가 다른 자리에 있으면 볼
- 같은 수가 전혀 없으면 포볼 이나 낫싱
### 게임 종료 
- 컴퓨터의 3자리 숫자와 플레이어의 3자리 숫자가 같은 경우 게임 종료 기능 ( 3 스트라이크 )
  - 게임을 재시작하려면 1을 입력
  - 게임을 완전히 종료하려면 2를 입력
  - 잘못된 값을 입력할 경우 IllegalArgumentException 발생한 후 애플리케이션 종료
    - 1또는 2가 아닌 숫자 또는 문자가 입력 되는 경우
